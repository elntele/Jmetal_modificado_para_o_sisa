...
public class EnvironmentalSelection<S extends Solution<?>> implements SelectionOperator<List<S>, List<S>>,
	...
	int FindNicheReferencePoint()
	{
		...
		// jorge candeias
//		como inicialmente min_size é igual a 214783647, e  referencePoint.MemberSize()
//		é o tamanho de um membro de da lista reference point 
//		(ps: não entendi muito bem se é lista de lista), e, 
//		Math.min(min_size,referencePoint.MemberSize()) retorna o menor de 2
//		valores (entre min_size e referencePoint.MemberSize()).
//		muito provavelmente Math.min(min_size,referencePoint.MemberSize()) 
//		ira retornar o valor de referencePoint.MemberSize(), colocando esse 
//		for para gerar um min_size igual ao referencePoint.MemberSize() 
		...
		// jorge candeias
//		esse for abaixo vai percorrer o array de referencepoints
//		na condição de ele não ter o seu tamanho igual a 0.
//		Caso ele não tenha, ou seja, caso o array reference points 
//		não seja vazio. ele percorre referencepoints até encontrar um
//		MemberSize com um valor igual a mim_size setado acima
//		então ele ira colocar no arrayList mim_rps este valor.
//		ps: trata-se de um unico inteiro
//		isso vai setar a lista mim_rps com o do indice r 
//		
		...
		int retorno = 0;
//		tratamento de excessão acrescentado para fins do sisa by jorge candeias
			try {
				retorno = min_rps.get(min_rps.size() > 1 ? JMetalRandom.getInstance().nextInt(0, min_rps.size()-1):0);
	...
	public List<S> execute(List<S> source) throws JMetalException {
		...
		while (source.size() < this.solutionsToSelect)
		{
			...
			//tratamento de excessão acrescentado para fins do sisa by jorge candeias
			try {
				chosen = SelectClusterMember(this.referencePoints.get(min_rp));
			} catch (IndexOutOfBoundsException e) {
